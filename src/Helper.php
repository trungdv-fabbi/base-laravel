...
class Helper {
    ...
    /**
     * get templates replace array
     * @var string
     */
    public function replaceParamInTemplate(string $templateFileName, array $patternReplacement): string
    {
        $template = File::get(__DIR__ . $this->ds . 'Stubs' . $this->ds . $templateFileName . '.stub');

        return str_replace(array_keys($patternReplacement), array_values($patternReplacement), $template);
    }
